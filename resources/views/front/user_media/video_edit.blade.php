                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex flex-wrap justify-content-between align-items-center">
                                <h5 class="card-title">{{ __('Chỉnh sửa video') }}</h5>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('user-video.update', $video->id) }}" method="post" id="create_form">
                                @csrf
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label>{{ __('Tiêu đề') }}:</label>
                                        <input type="text" name="title" value="{{ old('title', $video->title) }}" class="form-control @error('title')is-invalid @enderror" required>
                                        @error('title')
                                    ...
                                    <div class="form-group">
                                        <label>{{ __('Link') }}:</label>
                                        <input type="text" name="link" value="{{ old('link', $video->link) }}" class="form-control @error('link')is-invalid @enderror" required>
                                        @error('link')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
